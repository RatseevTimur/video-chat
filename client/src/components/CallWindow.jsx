...
import mask1 from '../assets/images/sunglasses.png';
import mask2 from '../assets/images/glasses.png';
 ...
 useEffect(() => {
  const loadResources = async () => {
    try {
      ...
      textureLoader.load(glassesSrc.url, (texture) => {
        texture.colorSpace = THREE.SRGBColorSpace;
        const geometry = new THREE.PlaneGeometry(glassesSrc.geometry.x, glassesSrc.geometry.y);
...
useEffect(() => {
  const detectAndPositionGlasses = async () => {
    if (!remoteVideo.current || !model || !glassesMesh) return;
    const video = remoteVideo.current.video;
    // console.log(video)
    // if (remoteVideo && video.readyState !== 4) return;
    ...
    if (faceEstimates.length > 0) {
      ...
      const scaleY = -0.01;
      const offsetX = 0.00;
      const offsetY = glassesSrc.offsetY;
      ...
      glassesMesh.position.z = glassesSrc.posZ;
