...
### Как mama@yandex.ru узнает, что вы написали?
1. Вы добавляете её почту в чат → ей уходит **письмо-приглашение** (если настроен `MAIL_URL`)
2. Вы пишете сообщение → ей уходит **письмо-уведомление** + шифротекст
3. Если она уже в приложении онлайн — сообщение приходит сразу по сокету

Без `MAIL_URL` письма печатаются в **терминал сервера** (для теста).

### Как доказать, что почта ваша?
Вход теперь **только после кода**:
1. Invite + почта → «Получить код»
2. Код приходит на почту (или в терминал сервера)
3. Ввод кода → аккаунт verified

Без кода войти нельзя.

### Почта как «БД»?
| Что | Где |
|-----|-----|
| Живой чат | RAM сервера |
| Уведомление «вам написали» | Письмо на Яндекс |
| Копия сообщения | Шифротекст в письме (Яндекс не читает) |
| Ключи | Только браузер |

Читать почтовый ящик через IMAP **не нужно** и **не делаем** (пароль от ящика не просим).  
Почта = уведомление + сейф для шифротекста, не парольная БД.

### Включить реальную почту (один раз на сервере)

Пароль приложения Яндекса (не основной пароль):

```bash
export MAIL_URL='smtps://ВАШ_ЛОГИН%40yandex.ru:ПАРОЛЬ_ПРИЛОЖЕНИЯ@smtp.yandex.ru:465'
./start.sh
```
